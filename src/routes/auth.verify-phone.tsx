...
function VerifyPhonePage() {
  const { user, sendVerificationPhone, verifyPhone, t, lang } = useAuth();
  ...
  if (!user) {
    return (
      <AuthLayout title={t("auth.verifyPhoneTitle")}>
    ...
    return (
      <AuthLayout title={t("auth.verifyPhoneTitle")}>
        ...
        <Link
          ...
          className="mt-4 inline-block font-semibold text-primary hover:underline"
        >
          {lang === "bn" ? "প্রোফাইল আপডেট" : "Update profile"}
        </Link>
      </AuthLayout>
    );
  }
  ...
  const send = async () => {
    setBusy(true);
    try {
      const c = await sendVerificationPhone();
      setCooldown(60);
      toast.success(
        (lang === "bn" ? "SMS পাঠানো হয়েছে। ডেমো OTP: " : "SMS sent. Demo OTP: ") + c,
      );
    } catch (e) {
  ...
  const verify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(lang === "bn" ? "ফোন যাচাই হয়েছে" : "Phone verified");
      nav({ to: "/account" });
    } catch (e2) {
  ...
  return (
    <AuthLayout
      ...
      subtitle={
        (lang === "bn" ? "কোড পাঠানো হবে: " : "We'll send a code to: ") + user.phone
      }
    >
